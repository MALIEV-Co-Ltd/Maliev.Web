...
public sealed class HeroLayoutSourceTests
{
    ...
    public void QuoteDropzoneRoutesSelectedFilesToQuoteEngine()
    {
        ...
        Assert.Contains(".landing-quote-dropzone-action {", styles, StringComparison.Ordinal);
        Assert.Contains("display: inline-flex;", styles, StringComparison.Ordinal);
        Assert.Contains("min-height: 22px;", styles, StringComparison.Ordinal);
        Assert.Contains("color: var(--muted);", styles, StringComparison.Ordinal);
        Assert.Contains("font-family: var(--font-mono);", styles, StringComparison.Ordinal);
        Assert.Contains("font-size: .78rem;", styles, StringComparison.Ordinal);
        Assert.Contains(".landing-quote-dropzone-action:hover", styles, StringComparison.Ordinal);
        Assert.Contains(".landing-quote-dropzone:focus-visible .landing-quote-dropzone-action", styles, StringComparison.Ordinal);
    ...
    public void HomeUsesFinishedManufacturingAndNewsSections()
    {
        ...
        Assert.Contains(".machine-configurator-step", styles);
        Assert.Contains(".machine-option-action", styles);
        Assert.Contains(".machine-addon-toggle", styles);
        Assert.Contains(".machine-addon-option.is-selected .machine-addon-toggle-knob", styles);
        ...
        Assert.Contains("machine-configurator-step", source);
        Assert.Contains("@Text(\"Choose machine size\", \"เลือกขนาดเครื่อง\")", source);
        Assert.Contains("@Text(\"Select one\", \"เลือก 1 รุ่น\")", source);
        ...
        Assert.Contains("MachineVariantStateLabel(variant)", source);
        Assert.Contains("25L air compressor", source);
        Assert.Contains("@Text(\"Add air supply\", \"เพิ่มชุดลม\")", source);
        Assert.Contains("aria-label=\"@MachineCompressorAriaLabel\"", source);
        Assert.Contains("machine-addon-toggle", source);
    ...
    public void HomeMachineConfiguratorLimitsMachineSelectionAndCompressorAddon()
    {
        ...
        Assert.Contains("MachineCompressorAriaLabel", machineSection);
        Assert.Contains("machine-addon-toggle", machineSection);
        Assert.Contains("MachineVariantStateLabel(variant)", machineSection);
        Assert.Contains("@Text(\"Choose machine size\", \"เลือกขนาดเครื่อง\")", machineSection);
        Assert.Contains("@Text(\"Add air supply\", \"เพิ่มชุดลม\")", machineSection);
    ...
    public void DirectWebGoogleUserSecretsOverrideSharedAspireSecrets()
    {
        ...
        Assert.Contains("Direct Web user-secrets still take priority", readme);
