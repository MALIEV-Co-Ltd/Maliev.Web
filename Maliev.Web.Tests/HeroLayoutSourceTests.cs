...
public sealed class HeroLayoutSourceTests
{
    ...
    public void HomeUsesFinishedManufacturingAndNewsSections()
    {
        ...
        Assert.Contains("Precision machined component prepared for aerospace-adjacent engineering review", source);
        Assert.Contains("Prototype device housing prepared for medical product development review", source);
        ...
        Assert.Contains("Choose the 30g pneumatic machine", source);
        Assert.Contains("Choose the 50g pneumatic machine", source);
    ...
    public void BlogPostsIncludeExpandedPracticalNotesAndPdfPrintAction()
    {
        ...
        Assert.Contains("category={Uri.EscapeDataString(normalizedCategoryKey)}", source);
