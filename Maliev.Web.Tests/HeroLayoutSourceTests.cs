...
public sealed class HeroLayoutSourceTests
{
    ...
    public void ServicePageExplainsSpecificQuotationReadiness()
    {
        ...
        Assert.Contains("service-answer-card has-media", source);
        Assert.Contains("service-answer-card-media", source);
        Assert.Contains("route.ImageUrl", source);
        Assert.Contains("DetailItemImageAlt(route)", source);
    ...
    public void HomeUsesFinishedManufacturingAndNewsSections()
    {
        ...
        Assert.Contains("fixtures, guards, brackets", source);
        Assert.Contains("Robotics", source);
        Assert.Contains("end effectors, mounts", source);
        Assert.Contains("Automotive", source);
        Assert.Contains("checks, jigs, samples", source);
        Assert.Contains("Electronics", source);
        Assert.Contains("enclosures, inserts", source);
        ...
        Assert.Contains("transition: box-shadow .24s ease-in-out;", styles);
        ...
        Assert.Contains("background: color-mix(in srgb, var(--blue-soft) 72%, var(--paper));", processStyles);
        Assert.Contains("border-radius: var(--radius);", processStyles);
        Assert.Contains("width: 28px;", processStyles);
        Assert.Contains("height: 28px;", processStyles);
        Assert.Contains("font-size: .88rem;", processStyles);
