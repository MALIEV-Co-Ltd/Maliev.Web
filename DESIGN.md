...
Vercel's website is the visual thesis of developer infrastructure made invisible — a design system so restrained it borders on philosophical. The page is overwhelmingly white (`#ffffff`) with near-black (`#171717`) text, creating a gallery-like emptiness where every element earns its pixel. This isn't minimalism as decoration; it's minimalism as engineering principle. MALIEV uses the same restraint, with Inter as the English interface voice and Noto Sans Thai preserved as the Thai fallback.
...
- JetBrains Mono for code, counters, and technical labels with OpenType `"liga"` globally
...
- **Selection Text** (`hsla(0, 0%, 95%, 1)`): Text selection highlight.
...
### Font Family
- **Primary**: `Inter`, with fallbacks: `Noto Sans Thai, Segoe UI, system-ui, sans-serif`
- **Thai**: `Noto Sans Thai`, with fallbacks: `Leelawadee UI, Tahoma, Segoe UI, system-ui, sans-serif`
- **Monospace**: `JetBrains Mono`, with fallbacks: `Noto Sans Thai, ui-monospace, SFMono-Regular, Consolas, monospace`
- **OpenType Features**: `"liga"` enabled globally; `"tnum"` for tabular numbers on specific captions.

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Inter | 48px (3.00rem) | 600 | 1.00–1.17 (tight) | 0 | Maximum clarity, billboard impact |
| Section Heading | Inter | 40px (2.50rem) | 600 | 1.20 (tight) | 0 | Feature section titles |
| Sub-heading Large | Inter | 32px (2.00rem) | 600 | 1.25 (tight) | 0 | Card headings, sub-sections |
| Sub-heading | Inter | 32px (2.00rem) | 400 | 1.50 | 0 | Lighter sub-headings |
| Card Title | Inter | 24px (1.50rem) | 600 | 1.33 | 0 | Feature cards |
| Card Title Light | Inter | 24px (1.50rem) | 500 | 1.33 | 0 | Secondary card headings |
...
| Body Semibold | Inter | 16px (1.00rem) | 600 | 1.50 | 0 | Strong labels, active states |
...
| Micro Badge | Inter | 7px (0.44rem) | 700 | 1.00 (tight) | normal | `text-transform: uppercase`, tiny badges |

### Principles
- **Clear spacing as identity**: Inter should run with `letter-spacing: 0`. Hierarchy comes from size, weight, line-height, and spacing between elements, not compressed tracking.
- **Ligatures everywhere**: Text enables OpenType `"liga"`. Ligatures aren't decorative — they're structural, creating tighter, more efficient glyph combinations.
...
- **Mono for identity**: JetBrains Mono with `"tnum"` or `"liga"` serves as the technical voice for codes, counters, and manufacturing labels. Use uppercase tracking sparingly for small technical labels only.
...
**Primary Dark (Inferred from Inter system)**
- Background: `#171717`
- Text: `#ffffff`
- Padding: 8px 16px
- Radius: 6px
...
- Inter 48px weight 600 for the metric
...
- **Clear text, expanded space**: Headline clarity is counterbalanced by generous surrounding whitespace. The typography is direct; the space around it is vast.
...
- Don't increase body text letter-spacing — body copy should stay at `letter-spacing: 0`
...
### Collapsing Strategy
- Hero: display 48px → scales down while keeping `letter-spacing: 0`
...
6. JetBrains Mono uppercase for technical labels, Inter for everything else
