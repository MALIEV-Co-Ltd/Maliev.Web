...
## Maliev.Web adaptation

This Apple system (fetched from getdesign.md / VoltAgent awesome-design-md) is the reference.
Maliev.Web applies it with these project specifics:

- **Fonts:** the site ships **Albert Sans** / **Geist** (sans), **JetBrains Mono** (mono labels and
  specs), and **Noto Sans Thai** (Thai) in place of SF Pro, which is Apple-proprietary. Geist is the
  closest SF-like display face.
- **Monochrome-first, proper contrast:** ink (#171717 / #1d1d1f) on white, with dark surfaces for
  the inverse. Blue (`--blue`) is the single interactive accent and is used sparingly — never on
  headlines, decorative fills, or large surfaces. Do not overuse it.
- **No decorative gradients** of any kind (matching Apple's zero-gradient system). Surfaces are flat
  neutral; the only permitted gradient is a whisper-subtle neutral fade on large ambient areas.
- Keep Thai parity for all customer-facing copy.
